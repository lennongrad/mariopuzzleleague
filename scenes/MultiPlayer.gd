...
func _ready():
	randomize()
	$Board1.connect("play_music", Callable(self, "do_play_music"))
	$Board2.connect("play_music", Callable(self, "do_play_music"))
...
func start(player_one_data, player_two_data):
	...
	var _err = $Board1.connect("has_won", Callable($Board2, "lose_game"))
	_err = $Board1.connect("has_lost", Callable($Board2, "win_game"))
	_err = $Board2.connect("has_won", Callable($Board1, "lose_game"))
	_err = $Board2.connect("has_lost", Callable($Board1, "win_game"))
...
func tick(p1, p2, save_data):
	...
	if $Board1.is_has_won:
		$Frame/Character1.win()
	if $Board1.is_has_lost:
		$Frame/Character1.loss()
	if $Board2.is_has_won:
		$Frame/Character2.win()
	if $Board2.is_has_lost:
...
func _process(_delta):
	...
	$Frame/Level1.text = str(int($Board1.get_velocity())).pad_zeros(2)
	$Frame/Level2.text = str(int($Board2.get_velocity())).pad_zeros(2)
	...
	if $Board1.star_timer > 0:
		$Frame.material.set_shader_parameter("active", true)
	else:
		$Frame.material.set_shader_parameter("active", false)
	if $Board2.star_timer > 0:
		$Frame/RightFrame.material.set_shader_parameter("active", true)
	else:
		$Frame/RightFrame.material.set_shader_parameter("active", false)
...
func _on_Board1_spawn_trash(combo, _chain, p):
	var trash = load("res://scenes/TrashMessage.tscn").instantiate()
	trash.position = p
	...
	$Frame/TrashPreview2.blocks = board2_trash_list
	$Frame/TrashPreview2.shown = board2_trash_list.size() - board2_trash_waiting
	trash.connect("done_travelling", Callable(self, "remove_Board2_trash"))
...
func _on_Board2_spawn_trash(combo, _chain, p): 
	...
	$Frame/TrashPreview1.blocks = board1_trash_list
	$Frame/TrashPreview1.shown = board1_trash_list.size() - board1_trash_waiting
	trash.connect("done_travelling", Callable(self, "remove_Board1_trash"))
...
func _physics_process(_delta):
	if not debug:
		return
	$Board1.start_timer += 1
	$Board2.start_timer += 1
